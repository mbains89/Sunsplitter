...
const FAILED_POLICY_CORRECTION_BRANCH = "ticket/0.30.1-rec-ratchet-02-policy-selftest-correction-r1";
const POLICY_CORRECTION_BRANCH = "ticket/0.30.1-rec-ratchet-02-policy-selftest-correction-r2";
...
const POLICY_CORRECTION_COMMIT_TITLE = "REC-RATCHET-02: make correction fixtures clean-clone stable";
...
const POLICY_CORRECTION_AUTHOR = "Sunsplitter Recovery Build <noreply@openai.com> 1787702400 -0500";
...
const POLICY_PROJECTION_SHA256 = "c326aef49b156f624fb67b9e6e6fa0eab1cf86f987bffcdad9ec355f811d2206";
...
const POLICY_CORRECTION_STATUS_SHA256 = "c3291c5dc6268fe7a41e2d36f0e19d39cf49746c01d457cd22dd287db42bbddc";
const POLICY_CORRECTION_STATUS_BLOB = "ca75a1565a02e3f32be1be0314aedea2c69a807a";
const POLICY_CORRECTION_STATUS_BYTES = 20264;
const POLICY_CORRECTION_RECORD_SHA256 = "cbd3a7b9a90cbd1e43bb6b4f69f363bcf170f04a7aabba813728f666b9701806";
const POLICY_CORRECTION_RECORD_BLOB = "f7a1e5b88120c24e50dca8551af1fe8edcc0cf2b";
const POLICY_CORRECTION_RECORD_BYTES = 15066;
...
const FAILED_POLICY_CORRECTIONS = Object.freeze([
  Object.freeze({ label: "C1", head: FAILED_POLICY_CORRECTION_C1_HEAD, tree: FAILED_POLICY_CORRECTION_C1_TREE }),
  Object.freeze({ label: "C2", head: FAILED_POLICY_CORRECTION_C2_HEAD, tree: FAILED_POLICY_CORRECTION_C2_TREE }),
  Object.freeze({ label: "C3", head: FAILED_POLICY_CORRECTION_C3_HEAD, tree: FAILED_POLICY_CORRECTION_C3_TREE })
]);
...
function policyCorrectionEvidence(ref) {
  ...
  const rejectFailed = failed => {
    if (!failed || failedLabels.has(failed.label)) return;
    failedLabels.add(failed.label);
    errors.push(`failed policy correction ${failed.label} identity is non-reusable`);
  };
  rejectFailed(FAILED_POLICY_CORRECTIONS.find(failed => ref === failed.head));
  ...
  for (const failed of FAILED_POLICY_CORRECTIONS) {
    if (commit.oid === failed.head || commit.tree === failed.tree) rejectFailed(failed);
  }
...
function futureEvidence(ref, protectedMerge) {
  const errors = [];
  let failedIdentityRejected = false;
  if (ref === FAILED_REC_02_R1_HEAD) {
    errors.push("failed REC-02 r1 identity is non-reusable");
    failedIdentityRejected = true;
  }
  ...
  if (!failedIdentityRejected && (commit.oid === FAILED_REC_02_R1_HEAD || commit.tree === FAILED_REC_02_R1_TREE)) {
    errors.push("failed REC-02 r1 identity is non-reusable");
    failedIdentityRejected = true;
  }
  if (failedIdentityRejected) {
    return {
      errors,
      oid: commit.oid,
      tree: commit.tree,
...
function evaluatePolicy(facts) {
  ...
  if (facts.eventName === "pull_request") {
    ...
    if (facts.headRef === FAILED_POLICY_CORRECTION_BRANCH) {
      errors.push("policy correction r1 route is failed and non-reusable");
...
function selfTest() {
  ...
  assert.notEqual(correction.oid, FAILED_POLICY_CORRECTION_C3_HEAD);
  assert.notEqual(correction.tree, FAILED_POLICY_CORRECTION_C3_TREE);
  ...
  for (const failed of FAILED_POLICY_CORRECTIONS) {
    const byHead = policyCorrectionEvidence(failed.head);
    assert.ok(byHead.errors.some(error => error.includes(`failed policy correction ${failed.label} identity is non-reusable`)));
  }
  ...
  const correctionRawMutations = [
    ...
    [Buffer.from(correctionText.replace(`author ${POLICY_CORRECTION_AUTHOR}`, `author Altered Build <noreply@openai.com> 1787702400 -0500`)), "correction author"],
    [Buffer.from(correctionText.replace(`committer ${POLICY_CORRECTION_AUTHOR}`, `committer ${POLICY_CORRECTION_AUTHOR.replace("1787702400", "1787702401")}`)), "correction timestamp"],
  ...
  const alternateRaw = Buffer.from(correctionText.replace(`author ${POLICY_CORRECTION_AUTHOR}`, `author Alternate Build <noreply@openai.com> 1787702400 -0500`));
  ...
  const frozenCorrectionR1Pr = { ...correctionPr, headRef: FAILED_POLICY_CORRECTION_BRANCH };
  assert.ok(evaluatePolicy(frozenCorrectionR1Pr).errors.some(error => error.includes("policy correction r1 route is failed and non-reusable")));
  structuredRejected += 1;
  ...
  structuredRejected += 1;
  const failedCorrectionC3Pr = prFacts({
    sha: genericMerge(
      FAILED_POLICY_CORRECTION_C3_TREE,
      [GATE_A_MERGE_SHA, FAILED_POLICY_CORRECTION_C3_HEAD],
      "Failed policy correction C3 merge fixture"
    ).oid,
    base: GATE_A_MERGE_SHA,
    head: FAILED_POLICY_CORRECTION_C3_HEAD,
    headRef: POLICY_CORRECTION_BRANCH
  });
  assert.ok(evaluatePolicy(failedCorrectionC3Pr).errors.some(error => error.includes("failed policy correction C3 identity is non-reusable")));
  structuredRejected += 1;
  for (const failed of FAILED_POLICY_CORRECTIONS) {
    const failedTreeOid = writeRawCommit(canonicalRawCommit(
      failed.tree,
      GATE_A_MERGE_SHA,
      POLICY_CORRECTION_AUTHOR,
      POLICY_CORRECTION_COMMIT_TITLE,
    ...
    assert.notEqual(failedTreeOid, failed.head);
    assert.ok(policyCorrectionEvidence(failedTreeOid).errors.some(
      error => error.includes(`failed policy correction ${failed.label} identity is non-reusable`)
    ));
    structuredRejected += 1;
  }
  ...
  const failedR1ByHead = futureEvidence(FAILED_REC_02_R1_HEAD, protectedMerge);
  assert.ok(failedR1ByHead.errors.some(error => error.includes("failed REC-02 r1 identity is non-reusable")));
  const failedR1TreeOid = writeRawCommit(canonicalRawCommit(
    FAILED_REC_02_R1_TREE,
    protectedMerge,
    FUTURE_AUTHOR,
    FUTURE_COMMIT_TITLE,
    []
  ));
  assert.notEqual(failedR1TreeOid, FAILED_REC_02_R1_HEAD);
  assert.ok(futureEvidence(failedR1TreeOid, protectedMerge).errors.some(
    error => error.includes("failed REC-02 r1 identity is non-reusable")
  ));
...
function taskForRoute(route) {
  if (route?.startsWith("rec-ratchet-02-policy-correction")) return "REC-RATCHET-02-POLICY-SELFTEST-CORRECTION-R2";
