...
She does not soften the prognosis. She softens nothing. The offer is presence — and, if you both decide, more.`;
  ...
  // WRITES: privacy exit sets romance.amara_tomas then existing +3 cohesion/affinity, then debt_notice — closes private hours
...
"You can leave," Amara says. "Or you can stay. Those are the only clean options."`;
    ...
    get choices() {
      ...
      return [
        { text: "Leave them the privacy they have claimed.", next: "romance_amara_tomas_privacy", effects: { cohesion: 3 }, affinity: { amara: 4, tomas: 4 } },
  ...
  // One-shot closer for the privacy leave. Existing romance.amara_tomas key; no new flag.
  romance_amara_tomas_privacy: {
    onEnter: () => {
      if (!state.romance.amara_tomas) {
        state.romance.amara_tomas = true;
        remember("You left Amara and Tomas the hydroponics bay. The private hours are closed.");
      }
      return "debt_notice";
    },
    text: `The bay door takes their voices with it.`,
    choices: [
      { text: "Keep walking.", next: "debt_notice" }
    ]
  },
