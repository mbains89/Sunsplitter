...
That is not the lock-suffix word. See `docs/SUN_DOCS_TIP_HONESTY_01.md`.
...
- **Amara-route PARKED.**
- PR 45 / draft PR 46 untouched. No remint of spent PRs 107–302.
...
| PR **#296** `SUN-COMPETENCE-WATCH-WIRE-01` | Merged. `competence_watch.jpg`. |
| PR **#295** `SUN-LEAD-TOGETHER-WIRE-01` | Merged. `observation_bridge_alt.jpg`. |
| PR **#293** `SUN-POST292-TIP-HONESTY-01` | Merged. Historical honesty after #292. |
...
|---|---|---|---|---|
| `SUN-HITL-WIRE-REMAP-01` | Apply the five already-named remaps in `docs/SUN_HITL_WIRE_01.md` (existing in-tree JPEGs only). Skip any remap that fights a scene comment or a playtest retarget. | `src/state.js` `sceneImages` (or the live image map), `docs/SUN_HITL_WIRE_01.md` receipt | Named keys match the remap table or an explicit skip; no new filenames; version-verify still runs; dead/unrecovered still honest | No JPEG gen; no Slot15 remint; no 0.36 OPEN; no Netlify |

`SUN-HITL-WIRE-ADD-KEYS-A-01` is **ALREADY_SATISFIED** on this tip (aftermath / berths / `prom_*` keys already in the live map). Do not remint.
...
`SUN-DAMAGE-CAUSE-PROPOSAL-01` is **ALREADY_SATISFIED** as paper (PR **#298**). Owner lock of a cause remains OPEN. Do not remint the proposal identity.
...
| `SUN-LETHAL-RESUME-DEATHBEAT-01` | Optional harness assert only on current saves (`sceneEntered` + `skipOnEnter`). Death beat already plays; do not skip to the post-death redirect and do not re-kill. | lethals module + one harness assert | Resume on a lethal scene shows the beat once; second resume does not re-kill; version-verify / living-cast checks still pass | No new lethals; no 0.36; no Netlify |
...
| `SUN-PLAYTEST-INTRO-BACK-ART-01` | ALREADY_SATISFIED (later roster-honesty retarget is #302, not a remint of this id) |
...
| Ticket | Evidence |
|---|---|
| `SUN-INTRO-SLIDE-ROSTER-HONESTY-01` | PR **#302** @ `212ee85d6a65` |
| `SUN-ARC-LIVING-3-WIRE-01` | PR **#301** @ `c0155247` |
| `SUN-DAMAGE-CAUSE-PROPOSAL-01` | PR **#298** paper |
| `SUN-COMPETENCE-WATCH-WIRE-01` | PR **#296** |
| `SUN-LEAD-TOGETHER-WIRE-01` | PR **#295** |
| `SUN-HITL-WIRE-ADD-KEYS-A-01` | ALREADY_SATISFIED on lane map (no PR remint) |
...
2. `SUN-EMBRYO-COUNT-SWEEP-01`
3. `SUN-LETHAL-RESUME-DEATHBEAT-01` (optional harness)
4. `SUN-ART-PLATE-LOOP-01` (docs brief only)
