# SUN-ROADMAP-UPCOMING-01

SOURCE main@8d23109b · RUNTIME f173bb63264acccef82d6db776cc20f236e3b312 · TASK SUN-ROADMAP-UPCOMING-01 · MODE docs

Docs-only upcoming-version table after PR **#364** TIP-HONESTY-363.
Head branch is `ticket/0.30.1-roadmap-upcoming-01`.

Does not mint 0.36. Does not invent OPEN 0.37. Does not certify. Does not deploy.
Last certified remains `0.28.1d`.
Paint on this lane stays `VERSION.md` first line `0.36` (**PAINT**, not OPEN).
No gameplay code. No JPEG. No Netlify.
Ignore Copilot leftovers **#297** and **#304** unless the owner names them.
Remint is HOLD Approve-only.

Lane: `version/0.30.1-main-reconcile-ci.1`
Live tip recorded here (pre-this-PR HEAD): `f173bb63264acccef82d6db776cc20f236e3b312`
Evidence: merge of PR **#364** `SUN-DOCS-TIP-HONESTY-363-01`.
Parents of that merge: `50ec4895168774dd9341633fb46ee6d364a81707` (#363 tip) and `199a969faab3d8304b7df24fd26297f3a4b08602` (#364 ticket head).
Floor: base tip `≥ 50ec4895` is met (`50ec4895` is a parent of `f173bb63`).

This cite is that pre-PR lane HEAD. It is not this docs PR's own merge SHA.
`main` stays `8d23109b63b844e0703fb36643f14b91b8800c90`. Do not close the lane onto main.

Lock line: `lane 0.30.1 · certified 0.28.1d · NO-PUBLISH · 0.36 PAINT`

## Why this ticket exists

First file of this ticket sat on `ticket/0.30.1-roadmap-upcoming-01` as dirty PR **#365** off `50ec4895`. PR **#364** then merge-committed the STATUS/QUEUE retarget. #365 went dirty and was closed once.

Pointer rewrite of STATUS/QUEUE to `50ec4895` is **ALREADY_SATISFIED** by #364. Do not remint #364.

This named branch is kept. STATUS/QUEUE on it are restored to the #364 lane bytes so the remaining unique work is this receipt. PR **#366** (`01b`) is closed as the wrong head name.

Historical receipts stay historical:
- `docs/SUN_DOCS_TIP_HONESTY_363.md` — #364 / cite `50ec4895`
- `docs/SUN_RECEIPT_VERIFY_MODES_01.md` — #363
- `docs/SUN_ROADMAP_TIP_SYNC_351.md` — #352 / cite `3aa2053f`
- `docs/SUN_DOCS_TIP_HONESTY_02.md` — #347 / cite `c3626434`

## Upcoming versions (labeled; not OPEN)

Codex numbering is authority. Do not reuse 0.31–0.33 for independent review, PC, or commercial itch.

| Number | Job | State |
|---|---|---|
| **0.28.1d** | Last certified baseline | CERTIFIED floor. |
| **0.30** | Packaging label on `main` | SHIPPED observation. `NO-PUBLISH`. |
| **0.30.1–0.35** | Recovery through private-package paper | LANDED ON VERSION LANE. Drain closed. `NOT_CERTIFIED`. |
| **0.36** | PC Readiness | **PAINT.** `pc_readiness_0_36: not opened`. Keep-versions-moving is not a PC-evidence mint. |
| **0.37** | External Review Pilot | PREP / labeled gate. Entry = 0.36 passed. **Not OPEN.** |
| **0.38** | Player Validation Cohort | After 0.37 P0 closed. |
| **0.39** | Commercial itch readiness | After 0.38. |
| **0.40** | Launch rehearsal / RC | After 0.39. Does not publish. |
| **1.0** | Public release | Separate owner go/no-go. |

This table does not mint, certify, deploy, or open a product ticket.

## Spent on or before this tip (do not remint)

| PR | Ticket | Meaning |
|---|---|---|
| **#364** | `SUN-DOCS-TIP-HONESTY-363-01` | Live tip. STATUS/QUEUE cite `50ec4895`. ROADMAP/LOCKS labeled incomplete there. |
| **#363** | `SUN-RECEIPT-VERIFY-MODES-01` | AGENTS receipt-path honesty + `docs/VERIFY_MODES.md`. |
| **#360** | `SUN-CREW-BOARD-FOLLOW-CLARITY-01` | First Crew tap shows living board. Keep `renderCrewPanel("lena")`. |
| **#352** | `SUN-ROADMAP-TIP-SYNC-351-01` | Receipt only. |
| **#351** | `SUN-CASCADE-ALLUSIVE-PAYOFF-01` | `flags.changeorders` on `reckon_summary`. Tomas "People were tier four." unspent. |
| **#349** | `SUN-HITL-WIRE-REMAP-01` | SKIP 1–3 / ALREADY_SATISFIED 4–5. |
| **#348** | `SUN-TITLE-CONTINUE-CREW-02` | Continue chip = saved visible living crew. |
| **#326** | `SUN-LETHAL-RESUME-DEATHBEAT-01` | Resume death-beat harness. |
| **#306** | `SUN-EMBRYO-COUNT-SWEEP-01` | Spoken 140,006 live. |
| **#366** | `SUN-ROADMAP-UPCOMING-01` `01b` head | CLOSED. Wrong branch name. |

## Holds kept

- **Do not invent OPEN 0.37.**
- **0.36 stays PAINT, not OPEN.** `VERSION.md` and `docs/version-lock.md` are not edited.
- **Remint HOLD Approve-only.**
- **No Netlify.** No pin remint.
- **Ignore #297 and #304** unless the owner names them. #297 ships corrupt `src/engine.js` PLACEHOLDER. Do not merge.
- **No Bot JPEG.** No gameplay, `src/**`, `css/**`, `index.html`, or image bytes.
- **No certify, tag, Release, or close-out onto `main`.**
- PR 45 / draft PR 46 untouched. Amara-route PARKED. ART-R2 broad campaign HELD.

## Pointer files this ticket edits

- `docs/SUN_ROADMAP_UPCOMING_01.md` (this receipt)

Not in the intended final diff vs live lane: `artifacts/PROJECT_STATUS.md`, `docs/TICKET_QUEUE.md`, `artifacts/ROADMAP.md`, `artifacts/LOCKS.md`, `VERSION.md`, `docs/version-lock.md`, `AGENTS.md`, gameplay, art plates, Netlify.

Stop after this merge. Do not start the next ticket. Do not invent OPEN 0.37.
